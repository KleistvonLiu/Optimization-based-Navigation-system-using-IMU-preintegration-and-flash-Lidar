...
addpath('D:\DA_JiangfengLIU\Optimization-based-Navigation-system-using-IMU-preintegration-and-flash-Lidar-main\attitudeFunctionMatlab_liub');
...
load('D:\DA_JiangfengLIU\Optimization-based-Navigation-system-using-IMU-preintegration-and-flash-Lidar-main\data\dataBase_FS04_Case020_5.mat')
...
enddata = length(accdata)-1;%15001;% %length(accdata);%length(acc)
...
%     
% end
for i = 1:size(eulAng_LB_est,2)
    
    eulAng_LB_est(:,i) = rad2deg(eulAng_LB_est(:,i));
    
end
%%%eulAng_LB_est = eulAng_LB_est.';
eulAng_LB_ref = downsample(eulAng_LB_ref,100);
...
posi_LB_L_ref = downsample(posi_LB_L_ref,100);
posi_LB_L_ref = posi_LB_L_ref';

vel_LB_L_ref = downsample(vel_LB_L_ref,100);
vel_LB_L_ref = vel_LB_L_ref';

eulAng_LB_ref = eulAng_LB_ref';

eulAng_NB_est = rad2deg(eulAng_NB_est);
...
% 求最大的差欧氏距离
e_max_norm_opt = max(vecnorm(P_LB_L(:,1:enddata)-posi_LB_L_ref(:,1:enddata)));
e_max_norm_kf = max(vecnorm(posi_LB_L_est(:,1:enddata)-posi_LB_L_ref(:,1:enddata)));
% max. error for each axis
e_max_x_opt = max(abs(P_LB_L(1,1:enddata)-posi_LB_L_ref(1,1:enddata)));
e_max_y_opt = max(abs(P_LB_L(2,1:enddata)-posi_LB_L_ref(2,1:enddata)));
e_max_z_opt = max(abs(P_LB_L(3,1:enddata)-posi_LB_L_ref(3,1:enddata)));
e_max_x_kf = max(abs(posi_LB_L_est(1,1:enddata)-posi_LB_L_ref(1,1:enddata)));
e_max_y_kf = max(abs(posi_LB_L_est(2,1:enddata)-posi_LB_L_ref(2,1:enddata)));
e_max_z_kf = max(abs(posi_LB_L_est(3,1:enddata)-posi_LB_L_ref(3,1:enddata)));
% 求最终欧氏距离差
% er5 = vecnorm(Ps(:,enddata)-posi_LB_L_ref(:,enddata));
% er6 = vecnorm(posi_LB_L_est(:,enddata)-posi_LB_L_ref(:,enddata));
% 求欧氏距离差的平均值
e_mean_norm_opt = sum(vecnorm(P_LB_L(:,1:enddata)-posi_LB_L_ref(:,1:enddata)))/enddata;
e_mean_norm_kf = sum(vecnorm(posi_LB_L_est(:,1:enddata)-posi_LB_L_ref(:,1:enddata)))/enddata;

e_mean_x_opt = mean(abs(P_LB_L(1,1:enddata)-posi_LB_L_ref(1,1:enddata)));
e_mean_y_opt = mean(abs(P_LB_L(2,1:enddata)-posi_LB_L_ref(2,1:enddata)));
e_mean_z_opt = mean(abs(P_LB_L(3,1:enddata)-posi_LB_L_ref(3,1:enddata)));
e_mean_x_kf = mean(abs(posi_LB_L_est(1,1:enddata)-posi_LB_L_ref(1,1:enddata)));
e_mean_y_kf = mean(abs(posi_LB_L_est(2,1:enddata)-posi_LB_L_ref(2,1:enddata)));
e_mean_z_kf = mean(abs(posi_LB_L_est(3,1:enddata)-posi_LB_L_ref(3,1:enddata)));
...
% !!! here is a problem, we should use the computed angles_ref,but it
% doesn't work somehow.
%angles_ref_N = rad2deg(eulAng_LB_ref);

en_max_norm_opt = max(vecnorm(Ps(:,1:enddata)-posi_NB_N_ref(:,1:enddata)));

en_max_x_opt = max(abs(Ps(1,1:enddata)-posi_NB_N_ref(1,1:enddata)));
en_max_y_opt = max(abs(Ps(2,1:enddata)-posi_NB_N_ref(2,1:enddata)));
en_max_z_opt = max(abs(Ps(3,1:enddata)-posi_NB_N_ref(3,1:enddata)));

en_mean_norm_opt = sum(vecnorm(Ps(:,1:enddata)-posi_NB_N_ref(:,1:enddata)))/enddata;

en_mean_x_opt = mean(abs(Ps(1,1:enddata)-posi_NB_N_ref(1,1:enddata)));
en_mean_y_opt = mean(abs(Ps(2,1:enddata)-posi_NB_N_ref(2,1:enddata)));
en_mean_z_opt = mean(abs(Ps(3,1:enddata)-posi_NB_N_ref(3,1:enddata)));

h(98) = figure('Name','Position in Node coordinate system');
subplot(3,2,1)
tSim = linspace(0,floor(enddata/10),enddata); 
plot(tSim, Ps(1,1:enddata),'red','Linewidth', 2); hold on;grid on;
plot(tSim, posi_NB_N_ref(1,1:enddata),'black:','Linewidth', 2); hold on;grid on;
%plot(tSim, posi_NB_N_est(1,1:enddata),'blue'); hold on;grid on;
title('Position estimation in Node coordinate system');
ylabel('x [m]','FontSize',50);set(gca,'FontSize',15);
%legend('opt.','ref.','KF.')
legend('opt.','ref.')
subplot(3,2,3)
plot(tSim, Ps(2,1:enddata),'red','Linewidth', 2); hold on;grid on;
plot(tSim, posi_NB_N_ref(2,1:enddata),'black:','Linewidth', 2); hold on;grid on;
%plot(tSim, posi_NB_N_est(2,1:enddata),'blue'); hold on;grid on;
%legend('opt.','ref.','KF.')
legend('opt.','ref.')

ylabel('y [m]','FontSize',50);set(gca,'FontSize',15);
subplot(3,2,5)
plot(tSim, Ps(3,1:enddata),'red','Linewidth', 2); hold on;grid on;
plot(tSim, posi_NB_N_ref(3,1:enddata),'black:','Linewidth', 2); hold on;grid on;
%plot(tSim, posi_NB_N_est(3,1:enddata),'blue'); hold on;grid on;
%legend('opt.','ref.','KF.')
legend('opt.','ref.')
...
subplot(3,2,2)
plot(tSim, Ps(1,1:enddata)-posi_NB_N_ref(1,1:enddata),'red','Linewidth', 2); hold on;grid on;
%plot(tSim, posi_NB_N_est(1,1:enddata)-posi_NB_N_ref(1,1:enddata),'blue'); hold on;grid on;
title('Position errors in Node coordinate system');
%legend('opt.','KF.')
legend('opt.')


ylabel('x (opt.-ref.) [m]','FontSize',50);set(gca,'FontSize',15);
subplot(3,2,4)
plot(tSim, Ps(2,1:enddata)-posi_NB_N_ref(2,1:enddata),'red','Linewidth', 2); hold on;grid on;
%plot(tSim, posi_NB_N_est(2,1:enddata)-posi_NB_N_ref(2,1:enddata),'blue'); hold on;grid on;
%legend('opt.','KF.')
legend('opt.')


ylabel('y (opt.-ref.) [m]','FontSize',50);set(gca,'FontSize',15);
subplot(3,2,6)
plot(tSim, Ps(3,1:enddata)-posi_NB_N_ref(3,1:enddata),'red','Linewidth', 2); hold on;grid on;
%plot(tSim, posi_NB_N_est(3,1:enddata)-posi_NB_N_ref(3,1:enddata),'blue'); hold on;grid on;
%legend('opt.','KF.')
legend('opt.')


ylabel('z (opt.-ref.) [m]','FontSize',50);set(gca,'FontSize',15);
xlabel('time [s]','FontSize',20);

h(97) = figure('Name','Norm of position difference(opt.-ref.) in Node coordinate system');
% subplot(3,2,1)
% tSim = linspace(0,500,enddata); 
plot(tSim, vecnorm(Ps(:,1:enddata)-posi_NB_N_ref(:,1:enddata)),'red','Linewidth', 2); hold on;grid on;
title('Norm of position difference(opt.-ref.) in Node coordinate system');
ylabel('x [m]','FontSize',50);set(gca,'FontSize',15);
%legend('optEst')

h(96) = figure('Name','Attitude (Euler angles) in Node coordinate system');
subplot(3,2,1)
plot(tSim, angles_N(1,1:enddata),'red','Linewidth', 2); hold on;grid on;
plot(tSim, angles_ref_N(1,1:enddata),'black:','Linewidth', 2); hold on;grid on;
%plot(tSim, eulAng_NB_est(1,1:enddata),'blue'); hold on;grid on;

title('Attitude (Euler angles) estimation in Node coordinate system');
ylabel('roll [degree]','FontSize',50);set(gca,'FontSize',15);
%legend('opt.','ref.','KF.')
legend('opt.','ref.')

subplot(3,2,3)
plot(tSim, angles_N(2,1:enddata),'red','Linewidth', 2); hold on;grid on;
plot(tSim, angles_ref_N(2,1:enddata),'black:','Linewidth', 2); hold on;grid on;
%plot(tSim, eulAng_NB_est(2,1:enddata),'blue'); hold on;grid on;
...
subplot(3,2,5)
plot(tSim, angles_N(3,1:enddata),'red','Linewidth', 2); hold on;grid on;
plot(tSim, angles_ref_N(3,1:enddata),'black:','Linewidth', 2); hold on;grid on;
%plot(tSim, eulAng_NB_est(3,1:enddata),'blue'); hold on;grid on;
...
subplot(3,2,2)
plot(tSim, angles_N(1,1:enddata)-angles_ref_N(1,1:enddata),'red','Linewidth', 2); hold on;grid on;
%plot(tSim, eulAng_NB_est(1,1:enddata)-angles_ref_N(1,1:enddata),'blue'); hold on;grid on;
%legend('opt.','KF.')
legend('opt.')

title('Attitude (Euler angles) errors');
ylabel('x (opt.-ref.) [degree]','FontSize',50);set(gca,'FontSize',15);
subplot(3,2,4)
plot(tSim, angles_N(2,1:enddata)-angles_ref_N(2,1:enddata),'red','Linewidth', 2); hold on;grid on;
%plot(tSim, eulAng_NB_est(2,1:enddata)-angles_ref_N(2,1:enddata),'blue'); hold on;grid on;
%legend('opt.','KF.')
legend('opt.')

ylabel('y (opt.-ref.) [degree]','FontSize',50);set(gca,'FontSize',15);
subplot(3,2,6)
plot(tSim, angles_N(3,1:enddata)-angles_ref_N(3,1:enddata),'red','Linewidth', 2); hold on;grid on;
%plot(tSim, eulAng_NB_est(3,1:enddata)-angles_ref_N(3,1:enddata),'blue'); hold on;grid on;
%legend('opt.','KF.')
legend('opt.')
ylabel('z (opt.-ref.) [degree]','FontSize',50);set(gca,'FontSize',15);
xlabel('time [s]','FontSize',20);

%en_max_norm_opt = max(vecnorm(Ps(:,1:enddata)-posi_NB_N_ref(:,1:enddata)));

ean_max_x_opt = max(abs(angles_N(1,1:enddata)-angles_ref_N(1,1:enddata)));
ean_max_y_opt = max(abs(angles_N(2,1:enddata)-angles_ref_N(2,1:enddata)));
ean_max_z_opt = max(abs(angles_N(3,1:enddata)-angles_ref_N(3,1:enddata)));

%en_mean_norm_opt = sum(vecnorm(Ps(:,1:enddata)-posi_NB_N_ref(:,1:enddata)))/enddata;

ean_mean_x_opt = mean(abs(angles_N(1,1:enddata)-angles_ref_N(1,1:enddata)));
ean_mean_y_opt = mean(abs(angles_N(2,1:enddata)-angles_ref_N(2,1:enddata)));
ean_mean_z_opt = mean(abs(angles_N(3,1:enddata)-angles_ref_N(3,1:enddata)));
%% visulization
h(1) = figure('Name','Position');
subplot(3,2,1)
tSim = linspace(0,floor(enddata/10),enddata); 
plot(tSim, posi_LB_L_est(1,1:enddata),'green','Linewidth', 2); hold on;grid on;
plot(tSim, P_LB_L(1,1:enddata),'red','Linewidth', 2); hold on;grid on;
plot(tSim, posi_LB_L_ref(1,1:enddata),'black:','Linewidth', 2); hold on;grid on;
title('Position Estimation in Local coordinate system');
ylabel('x [m]','FontSize',50);set(gca,'FontSize',15);
legend('KF.','opt.','ref.')
subplot(3,2,3)
plot(tSim, posi_LB_L_est(2,1:enddata),'green','Linewidth', 2); hold on;grid on;
plot(tSim, P_LB_L(2,1:enddata),'red','Linewidth', 2); hold on;grid on;
plot(tSim, posi_LB_L_ref(2,1:enddata),'black:','Linewidth', 2); hold on;grid on;
legend('KF.','opt.','ref.')
ylabel('y [m]','FontSize',50);set(gca,'FontSize',15);
subplot(3,2,5)
plot(tSim, posi_LB_L_est(3,1:enddata),'green','Linewidth', 2); hold on;grid on;
plot(tSim, P_LB_L(3,1:enddata),'red','Linewidth', 2); hold on;grid on;
plot(tSim, posi_LB_L_ref(3,1:enddata),'black:','Linewidth', 2); hold on;grid on;
legend('KF.','opt.','ref.')
ylabel('z [m]','FontSize',50);set(gca,'FontSize',15);
xlabel('time [s]','FontSize',20);

subplot(3,2,2)
plot(tSim, posi_LB_L_est(1,1:enddata)-posi_LB_L_ref(1,1:enddata),'green','Linewidth', 2); hold on;grid on;
plot(tSim, P_LB_L(1,1:enddata)-posi_LB_L_ref(1,1:enddata),'red','Linewidth', 2); hold on;grid on;
legend('KF.','opt.')

title('Position errors');
ylabel('x [m]','FontSize',50);set(gca,'FontSize',15);
subplot(3,2,4)
plot(tSim, posi_LB_L_est(2,1:enddata)-posi_LB_L_ref(2,1:enddata),'green','Linewidth', 2); hold on;grid on;
plot(tSim, P_LB_L(2,1:enddata)-posi_LB_L_ref(2,1:enddata),'red','Linewidth', 2); hold on;grid on;
legend('KF.','opt.')

ylabel('y [m]','FontSize',50);set(gca,'FontSize',15);
subplot(3,2,6)
plot(tSim, posi_LB_L_est(3,1:enddata)-posi_LB_L_ref(3,1:enddata),'green','Linewidth', 2); hold on;grid on;
plot(tSim, P_LB_L(1,1:enddata)-posi_LB_L_ref(1,1:enddata),'red','Linewidth', 2); hold on;grid on;
legend('KF.','opt.')
...
h(2) = figure('Name','Norm of position difference(opt.-ref.)');
% subplot(3,2,1)
% tSim = linspace(0,500,enddata); 
plot(tSim, vecnorm(posi_LB_L_est(:,1:enddata)-posi_LB_L_ref(:,1:enddata)),'r'); hold on;grid on;
plot(tSim, vecnorm(P_LB_L(:,1:enddata)-posi_LB_L_ref(:,1:enddata)),'green','Linewidth', 2); hold on;grid on;
title('Norm of position difference(opt.-ref.)');
ylabel('x [m]','FontSize',50);set(gca,'FontSize',15);
legend('KF.','opt.')
...
% subplot(3,2,1)
% %tSim = linspace(0,500,enddata); 
% plot(tSim, vel_LB_L_est(1,1:enddata),'green','Linewidth', 2); hold on;grid on;
% plot(tSim, Vs(1,1:enddata),'red','Linewidth', 2); hold on;grid on;
% plot(tSim, vel_LB_L_ref(1,1:enddata),'black:','Linewidth', 2); hold on;grid on;
% title('Velocity Estimation in Local coordinate system');
% ylabel('x [m/s]','FontSize',50);set(gca,'FontSize',15);
% legend('KF.','opt.','ref.')
% subplot(3,2,3)
% plot(tSim, vel_LB_L_est(2,1:enddata),'green','Linewidth', 2); hold on;grid on;
% plot(tSim, Vs(2,1:enddata),'red','Linewidth', 2); hold on;grid on;
% plot(tSim, vel_LB_L_ref(2,1:enddata),'black:','Linewidth', 2); hold on;grid on;
% legend('KF.','opt.','ref.')
% ylabel('y [m/s]','FontSize',50);set(gca,'FontSize',15);
% subplot(3,2,5)
% plot(tSim, vel_LB_L_est(3,1:enddata),'green','Linewidth', 2); hold on;grid on;
% plot(tSim, Vs(3,1:enddata),'red','Linewidth', 2); hold on;grid on;
% plot(tSim, vel_LB_L_ref(3,1:enddata),'black:','Linewidth', 2); hold on;grid on;
% legend('KF.','opt.','ref.')
...
% ylabel('x (opt.-ref.) [m/s]','FontSize',50);set(gca,'FontSize',15);
% subplot(3,2,4)
% plot(tSim, Vs(2,1:enddata)-vel_LB_L_ref(2,1:enddata)); hold on;grid on;
% ylabel('y (opt.-ref.) [m/s]','FontSize',50);set(gca,'FontSize',15);
% subplot(3,2,6)
% plot(tSim, Vs(3,1:enddata)-vel_LB_L_ref(3,1:enddata)); hold on;grid on;
% ylabel('z (opt.-ref.) [m/s]','FontSize',50);set(gca,'FontSize',15);
% xlabel('time [s]','FontSize',20);

h(4) = figure('Name','Attitude (Euler angles) estimation in Local coordinate system');
subplot(3,2,1)
plot(tSim, eulAng_LB_est(1,1:enddata),'green','Linewidth', 2); hold on;grid on;
plot(tSim, angles_L(1,1:enddata),'red','Linewidth', 2); hold on;grid on;
plot(tSim, rad2deg(eulAng_LB_ref(1,1:enddata)),'black:','Linewidth', 2); hold on;grid on;
title('Attitude (Euler angles) estimation in Local coordinate system');
ylabel('roll [degree]','FontSize',50);set(gca,'FontSize',15);
legend('KF.','opt.','ref.')
subplot(3,2,3)
plot(tSim, eulAng_LB_est(2,1:enddata),'green','Linewidth', 2); hold on;grid on;
plot(tSim, angles_L(2,1:enddata),'red','Linewidth', 2); hold on;grid on;
plot(tSim, rad2deg(eulAng_LB_ref(2,1:enddata)),'black:','Linewidth', 2); hold on;grid on;
legend('KF.','opt.','ref.')
ylabel('pitch [degree]','FontSize',50);set(gca,'FontSize',15);
subplot(3,2,5)
plot(tSim, eulAng_LB_est(3,1:enddata),'green','Linewidth', 2); hold on;grid on;
plot(tSim, angles_L(3,1:enddata),'red','Linewidth', 2); hold on;grid on;
plot(tSim, rad2deg(eulAng_LB_ref(3,1:enddata)),'black:','Linewidth', 2); hold on;grid on;
legend('KF.','opt.','ref.')
ylabel('yaw [degree]','FontSize',50);set(gca,'FontSize',15);
xlabel('time [s]','FontSize',20);

subplot(3,2,2)
plot(tSim, eulAng_LB_est(1,1:enddata)-rad2deg(eulAng_LB_ref(1,1:enddata)),'green','Linewidth', 2); hold on;grid on;
plot(tSim, angles_L(1,1:enddata)-rad2deg(eulAng_LB_ref(1,1:enddata)),'red','Linewidth', 2); hold on;grid on;
legend('KF.','opt.')

title('Attitude (Euler angles) errors');
ylabel('x [degree]','FontSize',50);set(gca,'FontSize',15);
subplot(3,2,4)
plot(tSim, eulAng_LB_est(2,1:enddata)-rad2deg(eulAng_LB_ref(2,1:enddata)),'green','Linewidth', 2); hold on;grid on;
plot(tSim, angles_L(2,1:enddata)-rad2deg(eulAng_LB_ref(2,1:enddata)),'red','Linewidth', 2); hold on;grid on;
legend('KF.','opt.')

ylabel('y [degree]','FontSize',50);set(gca,'FontSize',15);
subplot(3,2,6)
plot(tSim, eulAng_LB_est(3,1:enddata)-rad2deg(eulAng_LB_ref(3,1:enddata)),'green','Linewidth', 2); hold on;grid on;
plot(tSim, angles_L(3,1:enddata)-rad2deg(eulAng_LB_ref(3,1:enddata)),'red','Linewidth', 2); hold on;grid on;
legend('KF.','opt.')

ylabel('z [degree]','FontSize',50);set(gca,'FontSize',15);
xlabel('time [s]','FontSize',20);

%!!Caution: for sigular point of fs04case21
%eulAng_LB_est(3,18662) = eulAng_LB_est(3,18661)-rad2deg(eulAng_LB_ref(3,18661))+rad2deg(eulAng_LB_ref(3,18662));

% errors in attitude(Euler angles)

% ea_max_norm_opt = max(vecnorm(P_LB_L(:,1:enddata)-posi_LB_L_ref(:,1:enddata)));
% ea_max_norm_kf = max(vecnorm(posi_LB_L_est(:,1:enddata)-posi_LB_L_ref(:,1:enddata)));
% max. error for each axis
ea_max_x_opt = max(abs(angles_L(1,1:enddata)-rad2deg(eulAng_LB_ref(1,1:enddata))));
ea_max_y_opt = max(abs(angles_L(2,1:enddata)-rad2deg(eulAng_LB_ref(2,1:enddata))));
ea_max_z_opt = max(abs(angles_L(3,1:enddata)-rad2deg(eulAng_LB_ref(3,1:enddata))));
ea_max_x_kf = max(abs(eulAng_LB_est(1,1:enddata)-rad2deg(eulAng_LB_ref(1,1:enddata))));
ea_max_y_kf = max(abs(eulAng_LB_est(2,1:enddata)-rad2deg(eulAng_LB_ref(2,1:enddata))));
ea_max_z_kf = max(abs(eulAng_LB_est(3,1:enddata)-rad2deg(eulAng_LB_ref(3,1:enddata))));

% 求欧氏距离差的平均值
% ea_mean_norm_opt = sum(vecnorm(P_LB_L(:,1:enddata)-posi_LB_L_ref(:,1:enddata)))/enddata;
% ea_mean_norm_kf = sum(vecnorm(posi_LB_L_est(:,1:enddata)-posi_LB_L_ref(:,1:enddata)))/enddata;

ea_mean_x_opt = mean(abs(angles_L(1,1:enddata)-rad2deg(eulAng_LB_ref(1,1:enddata))));
ea_mean_y_opt = mean(abs(angles_L(2,1:enddata)-rad2deg(eulAng_LB_ref(2,1:enddata))));
ea_mean_z_opt = mean(abs(angles_L(3,1:enddata)-rad2deg(eulAng_LB_ref(3,1:enddata))));
ea_mean_x_kf = mean(abs(eulAng_LB_est(1,1:enddata)-rad2deg(eulAng_LB_ref(1,1:enddata))));
ea_mean_y_kf = mean(abs(eulAng_LB_est(2,1:enddata)-rad2deg(eulAng_LB_ref(2,1:enddata))));
ea_mean_z_kf = mean(abs(eulAng_LB_est(3,1:enddata)-rad2deg(eulAng_LB_ref(3,1:enddata))));


% h(5) = figure('Name','Ref. acceleration in L frame');
% subplot(3,1,1)
% plot(tSim, acc_LB_L_ref(1,1:enddata),'green','Linewidth', 2); hold on;grid on;
% % plot(tSim, refRN_Vel_BL(1,1:numStepSim),'black:','Linewidth', 2); hold on;grid on;
% title('Acc. in L-frame');
% ylabel('x [m/s^2]');
% % legend('Navigation','Reference')
% subplot(3,1,2)
% plot(tSim, acc_LB_L_ref(2,1:enddata),'green','Linewidth', 2); hold on;grid on;
% % plot(tSim, refRN_Vel_BL(2,1:numStepSim),'black:','Linewidth', 2); hold on;grid on;
% ylabel('y [m/s^2]');
% % legend('Navigation','Reference')
% subplot(3,1,3)
% plot(tSim, acc_LB_L_ref(3,1:enddata),'green','Linewidth', 2); hold on;grid on;
% % plot(tSim, refRN_Vel_BL(3,1:numStepSim),'black:','Linewidth', 2); hold on;grid on;
% ylabel('z [m/s^2]');
% % legend('Navigation','Reference')
% xlabel('time [s]');
% 
...
% subplot(3,1,1)
% % plot(tSim, rad2deg(angRate_LB_B_est(1,1:enddata)),'green','Linewidth', 2); hold on;grid on;
% plot(tSim, rad2deg(angRate_LB_B_ref(1,1:enddata)),'green','Linewidth', 2); hold on;grid on;
% title('INS angular rates LB in B');
% ylabel('phi [deg/s]');
% % legend('fLaINSest','Ref.')
% legend('Ref.')
% subplot(3,1,2)
% % plot(tSim, rad2deg(angRate_LB_B_est(2,1:enddata)),'green','Linewidth', 2); hold on;grid on;
% plot(tSim, rad2deg(angRate_LB_B_ref(2,1:enddata)),'green','Linewidth', 2); hold on;grid on;
% ylabel('theta [deg/s]');
% % legend('fLaINSest','Ref.')
% legend('Ref.')
% subplot(3,1,3)
% % plot(tSim, rad2deg(angRate_LB_B_est(3,1:enddata)),'green','Linewidth', 2); hold on;grid on;
% plot(tSim, rad2deg(angRate_LB_B_ref(3,1:enddata)),'green','Linewidth', 2); hold on;grid on;
% ylabel('psi [deg/s]');
% % legend('fLaINSest','Ref.')
% legend('Ref.')
% xlabel('time [s]');
...
%% check the PC
basepcindex = 1301;
pcindex = 4301;%4301 11301 for fs04ca20
...
xlim([-4 6])
set(gca,'XTick',[-4:1:6]) %改变x轴坐标间隔显示 这里间隔为2
ylim([-6 6])
zlim([6 10])
set(gca,'ZTick',[6:1:10])
...
% zlim([5 9]);
% for the "spacecraft body frame"
h(17).CurrentAxes.YDir = 'Reverse';
h(17).CurrentAxes.ZDir = 'Reverse';
h(17).CurrentAxes.XColor = 'red';
h(17).CurrentAxes.YColor = 'green';
h(17).CurrentAxes.ZColor = 'blue';
h(17).CurrentAxes.LineWidth = 3.0;
grid on;